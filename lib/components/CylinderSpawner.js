  ...
  update (dt) {
    ...
    this.meshPool.meshes.forEach(mesh => {
      ...
      if (mesh.isEmissive) {
        ...
        factor = f * mesh.emissiveFactor;
        factor += this.audio * 0.5;
      }
      mesh.material[mesh.materialIndex].uniforms.emissiveFactor.value = factor * this.emission;
      mesh.material[mesh.materialIndexOther].uniforms.emissiveFactor.value = this.emission;
    });
  }
}
