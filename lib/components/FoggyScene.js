...
module.exports = class FoggyScene extends THREE.Object3D {
  ...
  constructor (app) {
    ...
    this.spawners = [
      ...
      // new BGPlaneSpawner(this.app, this.data)
    ];
  ...
  setupSkybox () {
    ...
    const skyMaterial = new HDRMaterial({
      // depthTest: false,
      // depthWrite: false,
      // wireframe: true,
      // envMap: this.envMap,
  ...
  _updateFog () {
    ...
    this.traverse(child => {
      if (child.material && child.material.uniforms && child.material.uniforms.cameraMatrixWorld) {
        child.material.uniforms.cameraMatrixWorld.value.copy(this.app.camera.matrixWorld);
      }
      if (child.material && child.material.uniforms && child.material.uniforms.fogLightStrength) {
        child.material.uniforms.fogLightStrength.value = this.fogStrength;
      }
    });
  }
