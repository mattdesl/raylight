  ...
  constructor (app) {
    ...
    for (let y = 0; y < slices; y++) {
      for (let i = 0; i < segments; i++) {
        const t0 = (i / (segments - 1)) * 2.0 * Math.PI;
        const t1 = (y / (slices - 1)) * 2 - 1;
        const scale = 1;
        const zScale = Math.sqrt(1.0 - t1 * t1) * scale;

        const px = Math.cos(t0) * zScale;
        const pz = Math.sin(t0) * zScale;
        const py = t1 * scale;
        
        points.push({
          position: new THREE.Vector3(px, py, pz),
          speed: randomFloat(0.05, 2),
    ...
    const knots = newArray(knotCount).map(() => {
      const position = new THREE.Vector3().fromArray(randomSphere([], randomFloat(0, sphereRadius * 2)));
      return {
    ...
    const attractPoint = (p, knot, dist) => {
      ...
      tmpVec.multiplyScalar(p.speed * strength);
    ...
    this.meshes = clusters.map((cluster, i) => {
      ...
      const geometry = createRibbonGeometry(clusterPoints, 0.5, 0.035);
    ...
    //   pointGoemetry.vertices = geometry.vertices;
...
function createRibbonGeometry (points, w, h) {
