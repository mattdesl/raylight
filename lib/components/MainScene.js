...
const { randomFloat, shuffle, randomSphere, randomQuaternion, nextSeed, getSeed } = require('../util/random');
...
module.exports = class MainScene extends THREE.Object3D {
  ...
  save () {
    ...
    this.app.width = outputWidth;
    this.app.height = outputHeight;
    renderer.setPixelRatio(1);
    renderer.setSize(this.app.width, this.app.height);
    this.app.tick();
    this.app.render(this.outTarget);
    ...
    pngStream(renderer, this.outTarget)
      .pipe(concatStream(buffer => {
        const fileStream = streamSaver.createWriteStream(`${getSeed()}-${clicks}.png`);
  ...
  createMeshes () {
    ...
    const colors = ["#2d2d29", "#3ca2a2", "#dfece6"];
    // const colors = ['#58ACAE', '#2E302B', '#F8F7F8']
    // const colors = shuffle(palettes[Math.floor(randomFloat(1) * palettes.length)]).slice(0, 0);
    // colors.unshift('#2d2d29', '#fff')
    // colors.unshift('hsl(0, 0%, 10%)', 'hsl(0, 0%, 98%)')
    const weights = getUniformWeights(colors.length, [ 0.4, 0.4 ])
    ...
    const createVolumeLine = (spline, steps = 10, w = randomFloat(0.1, 0.95), h = randomFloat(0.015, 0.05), color) => {
    ...
    const layers = Math.floor(randomFloat(30, 100));
    for (let i = 0; i < layers; i++) {
      const thickness = 0.01;
      const steps = Math.floor(randomFloat(1) > 0.5 ? randomFloat(10, 50) : randomFloat(10, 200));
      const speed = 0.01;
      const r = randomFloat(0, 5);
      ...
      //     (randomFloat(1) > 0.5 ? 1 : -1) * randomFloat(1, 2),
      //     0,
      //     (randomFloat(1) > 0.5 ? 1 : -1) * randomFloat(1, 2));
      // } else {
      //   mesh.renderOrder = 1;
